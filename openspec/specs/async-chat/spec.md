...
Un membro di una lobby attiva SHALL poter inviare un messaggio testuale alla chat della lobby. Il messaggio SHALL essere associato al mittente e registrato con il timestamp server-side.

#### Scenario: Invio avvenuto con successo
- **WHEN** un membro di una lobby attiva invia un messaggio con contenuto testuale non vuoto
- **THEN** il sistema persiste il messaggio con sender_id, lobby_id e sent_at, e lo rende disponibile agli altri membri al prossimo polling

#### Scenario: Messaggio vuoto
- **WHEN** un membro tenta di inviare un messaggio con contenuto vuoto o solo spazi
